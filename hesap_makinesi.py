...
from hm_arayuz import Ui_MainWindow
...
class Hm_Arayuz(QtWidgets.QMainWindow,Ui_MainWindow):
    ...
    def frakam_basma(self):
        ...
        elif(self.pushButton_arti.isChecked() or self.pushButton_eksi.isChecked() or self.pushButton_bol.isChecked() or self.pushButton_carp.isChecked() and (not self.ikincisayi)):
            self.label.setText(format(float(buton.text()),'.15g'))
            self.ikincisayi = True
        else:
